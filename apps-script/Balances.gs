...
function computeBalance_(playerKeyValue) {
  ...
  return { owed: owed, paid: paid, balance: round2_(owed - paid), games: games };
}
...
function getPlayerBalance(payload) {
  ...
  var settlements = getSettlementsForPlayer_(payload.player_key);
  var settledAt = lastSettlementAt_(settlements);

  // Games played after the most recent settlement are what the player still owes.
  var unpaid = calc.games
    .filter(function (g) {
      return !settledAt || new Date(g.timestamp) > settledAt;
    })
    .sort(function (a, b) {
